...
#output.filename = sprintf("results/distance_table_%s_MFW_wurzburg", mfws)
...
#### MFW = 500
grep("Anonym_Enfant", rownames(frq.table))

imposters = imposters(reference.set = frq.table[-c(1), 1:500], test = frq.table[1,1:500], distance="wurzburg")
imposters
## NEU
#Mirabeau 	 0.21
#Nerciat 	 0.23
...
optim_imposters = imposters.optimize(frq.table[, 1:500])
optim_imposters
# output = parameter p1 and p2

optim_imposters_w = imposters.optimize(frq.table[, 1:500], distance="wurzburg")
optim_imposters_w
...
text_to_be_tested = frq.table[1,] # Anonym_Enfant
## Check PL
grep("PigaultLebrun_Adele", rownames(frq.table))
my_candidate = frq.table[71:74,]
my_imposters = frq.table[-c(1, 71:74),]
imp_refs = imposters(reference.set = my_imposters, test = text_to_be_tested, candidate.set = my_candidate, features=0.5)
imp_refs

## Check Mirabeau
grep("Mirabeau_Conversion", rownames(frq.table))
mirabeau = frq.table[65:66,]
mirabeau_imps = frq.table[-c(1, 65:66),]
